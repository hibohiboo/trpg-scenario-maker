Feature: キャラクター関係管理
  ...
  Scenario: キャラクター関係の基本操作
    ...
    And 関係名に "守る" と入力する
    And 作成フォームの "作成" ボタンをクリックする
    Then 関係性一覧に "勇者 → 姫: 守る" が表示される

    # 関係性を編集
    When 関係性 "勇者 → 姫: 守る" の編集ボタンをクリックする
    And 関係名を "愛している" に変更する
    And 編集フォームの "更新" ボタンをクリックする
    Then 関係性一覧に "勇者 → 姫: 愛している" が表示される
    ...
    And 作成フォームの "作成" ボタンをクリックする
    Then 関係性一覧に "姫 → 勇者: 慕っている" が表示される
    And 関係性一覧に "勇者 → 姫: 愛している" が表示される

    # 勇者→姫の関係性を削除
    When 関係性 "勇者 → 姫: 愛している" の削除ボタンをクリックする
    And 削除確認モーダルで "削除" ボタンをクリックする
    Then 関係性一覧に "勇者 → 姫: 愛している" が表示されない
    And 関係性一覧に "姫 → 勇者: 慕っている" が表示される

    # 姫→勇者の関係性を削除
    When 関係性 "姫 → 勇者: 慕っている" の削除ボタンをクリックする
    And 削除確認モーダルで "削除" ボタンをクリックする
    Then 関係性一覧に "姫 → 勇者: 慕っている" が表示されない
