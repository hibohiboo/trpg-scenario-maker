Feature: シナリオ×キャラクター関係管理
  ...
  Scenario: シナリオごとに異なる関係性を管理できる
    Given テスト用のシナリオ "続・冒険の始まり" が存在する
    And シナリオ "冒険の始まり" にキャラクター "勇者" (役割: "主人公") が登録されている
    And シナリオ "冒険の始まり" にキャラクター "姫" (役割: "協力者") が登録されている
    And シナリオ "冒険の始まり" に関係性 "勇者" から "姫" への "守る" が登録されている
    And シナリオ "続・冒険の始まり" に既存キャラクター "勇者" (役割: "主人公") を追加している
    And シナリオ "続・冒険の始まり" に既存キャラクター "姫" (役割: "王") を追加している
    When シナリオ "続・冒険の始まり" の詳細ページを開く
    And "関係性を追加" ボタンをクリックする
    And 関係元キャラクターで "勇者" を選択する
    And 関係先キャラクターで "姫" を選択する
    And 関係名に "仕える" と入力する
    And モーダルの "追加" ボタンをクリックする
    Then シナリオ "続・冒険の始まり" の関係性に "勇者" から "姫" への "仕える" が表示される
    And シナリオ "冒険の始まり" の関係性に "勇者" から "姫" への "守る" が表示される
