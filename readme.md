...
- シナリオへの登場キャラクター設定
- **キャラクター関係グラフの可視化**
  - Cytoscape.jsによる高度なグラフ表示
  - 相互関係のエッジは自動的にカーブして表示（ラベルの被りを解決）
  - 3種類のレイアウトアルゴリズム：
    - 自動配置 (cose): 力学ベースのレイアウト
    - 階層表示 (breadthfirst): ツリー構造の階層表示
    - 円形配置 (circle): 円形に配置
  - エッジクリックでハイライト表示
  - マウスホイールでズーム、ドラッグで画面移動
...
シナリオを完全なデータとしてバックアップし、別環境で復元できます。
...
- **エクスポート機能**
  - シナリオ全体をZIP形式でダウンロード
  - 含まれるデータ：
    - シナリオメタデータ（タイトル、作成日時など）
    - 全シーン・シーンイベント
    - 全キャラクター・キャラクター関係性
    - 全情報項目・情報項目の関連性
    - 全画像データ（Data URL形式）
    - グラフ構造（シーン接続、キャラクター関係など）
  - ファイル名: `scenario_[シナリオタイトル].zip`

- **インポート機能**
  - ZIPファイルをアップロードして新規シナリオとして復元
  - **ID自動再発番**: 既存データとの衝突を回避
  - データ整合性の保証：全エンティティの関連性を維持したまま復元
...
- 複数デバイス間でのシナリオ共有
- テンプレートシナリオの配布・再利用
- シナリオの複製（同じ構造で別シナリオを作成）

**使用技術:**

- [@zip.js/zip.js](https://github.com/gildas-lormeau/zip.js) - 現代的なZIP圧縮/解凍ライブラリ
